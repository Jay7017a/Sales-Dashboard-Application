		...
		<>
			<div className={`${classes.container} relative bg-slate-50 dark:bg-slate-900 transition-colors duration-300`}>
				<div className='absolute top-4 right-4 z-50'>
					...
					>
						{theme === "dark" ? (
							...
							<img src={logos.moonIcon} alt='Moon Icon' className='size-7 text-gray-700' />
						)}
					</AnimatedButton>
				</div>
				<motion.div
				...
				>
					...
					>
						<div className={`${classes.form_label} text-2xl text-sky-500 dark:text-sky-300`}>Sign Up</div>
						<form className={classes.form} onSubmit={handleSubmit}>
							<div className='flex flex-col align-center'>
								<input
									...
									placeholder='Name'
								/>
								{errors.username && <span className='text-red-500 text-xs self-start ml-2'>{errors.username}</span>}
							</div>
							<div className='flex flex-col align-center'>
								<input
									type='text'
							...
							<motion.button
								...
								whileTap={{ scale: 0.95 }}
								disabled={state.isLoading}
								className={`${classes.btn} bg-sky-600 w-52 h-12 rounded-full dark:bg-sky-700 hover:bg-sky-700 dark:hover:bg-sky-600 border-sky-600 dark:border-sky-500`}
								type='submit'
							>
						...
						<Link to={"/login"}>
							<motion.button
								whileHover={{ scale: 1.05 }}
								whileTap={{ scale: 0.95 }}
								className={`${classes.btn}   bg-sky-600 w-48 h-11 rounded-full dark:bg-sky-800 hover:bg-sky-800 dark:hover:bg-sky-600 `}
							>
